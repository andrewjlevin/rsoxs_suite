...
# import multiprocess as mp  # for macos parallelization
...
def seed_worker():
    '''
    Generates unique seeds for numpy.random so multiprocessing does not give same random numbers
...
def generate_and_save_scat_coords(e_dens, length, unit_length, diameter, sigma,fuzz_dens,fuzz_length, save_dir, i):
    ...
    '''
    coords = gen_scat_coords_flexcyl(e_dens,length,unit_length,diameter,sigma,fuzz_dens,fuzz_length,save_dir,i,verbose=False)
    save_name = 'flex_cylinder_num' + str(i)
    np.save(save_dir + save_name + '.npy', coords)

def parallel_gen_scat_coords(num_fibrils,e_dens,length,unit_length,diameter,sigma,fuzz_dens,fuzz_length,save_dir):
    '''
    Generates scattering coordinates array for multiple fibrils of the same characteristics. 
    Wrapper for generate_and_save_scat_coords() utilizing multiprocessing pool
    ...
    Inputs
      ...
      save_dir - string save directory
    '''
    if num_fibrils > mp.cpu_count():
        num_process = mp.cpu_count()
    else:
        num_process = num_fibrils
    with mp.Pool(processes=num_process, initializer=seed_worker) as pool:
        results = [pool.apply_async(generate_and_save_scat_coords,
                                    args=(e_dens,length,unit_length,diameter,sigma,fuzz_dens,fuzz_length,save_dir,i))
                   for i in range(num_fibrils)]
        
        for result in results:
            result.get()  # Wait for each process to complete
            
def test_fibril_scattering_par(num_fibrils,length,diameter,sigma,e_dens,fuzz_dens,fuzz_length,q_norm_idx,exp_path,save_dir):
    '''
    ...
    with open(file_path, "w") as file:
        ...
        file.write(f"fuzz_dens: {fuzz_dens}\n")
        file.write(f"fuzz_length: {fuzz_length}\n")
    ...
    parallel_gen_scat_coords(num_fibrils,e_dens,length,unit_length,diameter,sigma,fuzz_dens,fuzz_length,save_dir)
    ...
    for i in range(num_fibrils):
        ...
        ints = sq(coords, qs)
        # ints = ints/num_scatterers
        # q_norm = exp_data[q_norm_idx,1]/ints[q_norm_idx]
        # ints = ints*q_norm
...
if __name__=='__main__':
    ...
    # exp_path = '/projects/thch7683/test_files/PM7_CN_sol618_buf617_emp614_log_thin.txt'
    scriptPath = pathlib.Path('/Users/andrew/Library/CloudStorage/OneDrive-UCB-O365/research/data_analysis/rsoxs_suite/morph_gen/custom_fibril_gen')
    savePath = scriptPath.joinpath('nofuzz_outputs_v2/')
    # expPath = scriptPath.joinpath('PM6_CN_sol616_buf615_emp614.txt')
    expPath = scriptPath.joinpath('PM6_CF_sol636_buf635_emp614.txt')
    ...
    flexes_dict = {0.01:'0p01', 0.001:'0p001', 0.0001:'0p0001'}
    for flex in flexes:
        for total_diameter in total_diameters:
            # for fuzz_length in fuzz_lengths:
            diameter = total_diameter - 2*fuzz_length
            flex_label = flexes_dict[flex]
            test_fibril_scattering_par(
                num_fibrils, length, diameter, flex, e_dens,fuzz_dens, fuzz_length, q_norm_idx, exp_path,
                savepath + f'/dtotal{total_diameter}_length{length}_flex{flex_label}_fuzz{fuzz_length}_fuzzdens10to1/'
                )
# New, modified checkh5 from Pete Dudenas and Dean DeLongchamp, ca. March 2022, to allow compatibility with Euler morphologies. Also contains more helpful error messages
...
def checkH5(filename='perp82.hd5',z_slice = 0, subsample = None, outputmat = None, runquiet = False, plotstyle='light',scale=1.0,save_output = False,png_out_folder='',png_out_title=''):
    ...
    if not runquiet:
        print(f'Dataset dimensions: {zdim} × {ydim} × {xdim}')
...
#         if plotstyle == 'dark':
        ...
        for i in range(0,num_mat):    
            ...
            fig = plt.figure(figsize=(22*scale, 33*scale))
            ...
            Vfrac_cbar.set_label('Vfrac: volume fraction', rotation=270, labelpad = 22*scale)
            ...
            S_cbar.set_label('S: orientational order parameter', rotation=270, labelpad = 22*scale)
            ...
            theta_cbar.set_label('theta in radians', rotation=270, labelpad = 22*scale)
            ...
            psi_cbar.set_label('psi in radians', labelpad = 22*scale)       
            ...
            ax4i = inset_axes(ax6,  axes_class = matplotlib.projections.get_projection_class('polar'), width = 2*scale, height = 2*scale, axes_kwargs={"alpha": 0})
            ...
            ax5i = inset_axes(ax5,  axes_class = matplotlib.projections.get_projection_class('polar'), width = 2*scale, height = 2*scale, axes_kwargs={"alpha": 0})
            ...
            if save_output == True:
                plt.savefig(f'{png_out_folder}{png_out_title}_Mat{i+1}.png')
...
def plotpsi(filename='perp82.hd5',z_slice = 0, subsample = None, ax_to_plot= None, outputmat = None, runquiet = False, plotstyle='light',total_phys_imsizenm=4000.,res=2048.,
           scalebar_lengthnm=100.):
    ...
    if subsample is None:
        ...
        # number of nm per pixel
        nm_per_pix = total_phys_imsizenm / res
        # if you want a 100 nm scale bar, how many pixels? 
        bar_inpixels = int(round(scalebar_lengthnm / nm_per_pix))
...
#         if plotstyle == 'dark':
        ...
        for i in range(0,1):  # plotting for TPD only for these purposes
            ...
            norm=matplotlib.colors.Normalize(vmin=0, vmax=np.pi, clip=False)
            psiplot = ax_to_plot.imshow(np.ma.masked_array(psi[i, z_slice, start:end, start:end], np.logical_or(Vfrac[i, z_slice, start:end, start:end]<0.01, S[i, z_slice, start:end, start:end]<0.01)), 
                                 cmap = plt.get_cmap('hsv'), norm=norm, origin='lower',interpolation='none')
            
            scalebar = AnchoredSizeBar(ax_to_plot.transData,
                           bar_inpixels, '100 nm', 'lower center', 
                           pad=0.1,
                           color='k',
                           frameon=False,
                           size_vertical=1)
            ax_to_plot.add_artist(scalebar)
...
def plottheta(filename='perp82.hd5',z_slice = 0, subsample = None, ax_to_plot= None, outputmat = None, runquiet = False, plotstyle='light',total_phys_imsizenm=4000.,res=2048.,
           scalebar_lengthnm=100.):
        #Can plot with light or dark background
    style_dict = {'dark':'dark_background',
                  'light':'default'}
...
#         if plotstyle == 'dark':
        ...
        for i in range(0,1):  # plotting for TPD only for these purposes
            ...
            norm=matplotlib.colors.Normalize(vmin=0, vmax=np.pi, clip=False)
            thetaplot = ax_to_plot.imshow(np.ma.masked_array(theta[i, z_slice, start:end, start:end], np.logical_or(Vfrac[i, z_slice, start:end, start:end]<0.01, S[i, z_slice, start:end, start:end]<0.01)), 
                                 cmap = plt.get_cmap('hsv'), norm=norm, origin='lower',interpolation='none')
            ...
            psi_cbar = plt.colorbar(thetaplot, fraction=0.040)
            psi_cbar.set_label('theta in radians', labelpad = 22)       

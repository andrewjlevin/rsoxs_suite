...
def center_sphere3D(NumXYZ, PhysSize, radius,interpolation=False,scale=2):
    if radius > PhysSize*NumXYZ/2:
        warnings.warn('Sphere diameter is greater than simulation box width')
    if interpolation:
        ...
        P = np.zeros((NumXYZ,NumXYZ,NumXYZ))
        ...
        P_subset[mask] = 1
        P = np.zeros((NumXYZ,NumXYZ,NumXYZ))
        low = int(P.shape[0]/2) - int(P_subset.shape[0]/2)
        high = int(P.shape[0]/2) + int(P_subset.shape[0]/2)
        P[low:high,low:high,low:high] = P_subset
    return P
...
def write_euler_hdf5(material_list, PhysSize, fname, ordering='ZYX', author='SpongeBob'):
    print(f'--> Marking {fname}')
    with h5py.File(fname,'w') as f:
...
def write_config(energies, angles, CaseType, MorphologyType):
        f = open('config.txt', "w")
        f.write("CaseType = " + str(CaseType) + ";\n");
        f.write("Energies = " + str(energies) + ";\n");
        f.write("EAngleRotation = " + str(angles) + ";\n");
